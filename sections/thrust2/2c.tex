...
\stefan{What are we implementing for 2b? Are we proposing an actual working runtime monitor? Would this require custom GPU drivers? (Presumably a non-starter unless we're working with NVIDIA)? Maybe a CUDA library that wraps read and write operations and does the tracking? Either way, this would be more than an OCaml implementation.}
